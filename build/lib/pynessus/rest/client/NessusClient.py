...
class NessusClient:
    ...
    def requestNessus(self, url, jsonFormat=True, method="GET"):
        '''
    ...
    def login(self, nessusUser, nessusPassword, jsonFormat=True):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['login'], jsonFormat=jsonFormat, method="POST")
    ...
    def feed(self, jsonFormat=True, method="GET"):
        '''
        Logout function to destroy a token created previously.
        Returns None if there's no token loaded in the class.
        '''
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['feed'], jsonFormat=jsonFormat, method=method)
        return content

    def securesettingsList(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_securesettings_list'], jsonFormat=jsonFormat, method=method)
        return content

    def secureSettings(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_securesettings'], jsonFormat=jsonFormat, method=method)
        return content

    def serverPreferencesList(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_preferences_list'], jsonFormat=jsonFormat, method=method)
        return content


    def serverPreferences(self,jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_preferences'], jsonFormat=jsonFormat, method=method)
        return content

    def serverUpdate(self,jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_update'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def serverRegister(self, nessusFeed, jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['server_register'], jsonFormat=jsonFormat, method=method)
        return content

    def serverLoad(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_load'], jsonFormat=jsonFormat, method=method)
        return content

    def serverUuid(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_uuid'], jsonFormat=jsonFormat, method=method)
        return content

    def serverGetCert(self,jsonFormat=False, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_getcert'], jsonFormat=jsonFormat, method=method)
        return content

    def serverPluginsProcess(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['server_plugins_process'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def usersAdd(self,login, password, admin=False, jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['users_add'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def usersDelete(self,login,jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['users_delete'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def usersChpasswd(self,login,password,jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['users_chpasswd'], jsonFormat=jsonFormat, method=method)
        return content

    def usersList(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['users_list'], jsonFormat=jsonFormat, method=method)
        return content

    def pluginsList(self,jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_list'], jsonFormat=jsonFormat, method=method)
        return content

    def pluginsAttributesList(self,jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_attributes_list'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def pluginsListFamily(self, pluginFamily, jsonFormat=True, method="GET"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_list_family'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def pluginsDescription(self, fileNamePlugin, jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_description'], jsonFormat=jsonFormat, method=method)
        return content

    def pluginsPreferences(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_preferences'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def pluginsAttributesFamilySearch(self, filter0Quality, filterSearchType, filter0Value, filter0Filter, jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_attributes_familySearch'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def pluginsAttributesPluginSearch(self,filter0Quality,filterSearchType,filter0Value,filter0Filter,family,jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_attributes_pluginSearch'], jsonFormat=jsonFormat, method=method)
        return content

    def pluginsMd5(self,jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_md5'], jsonFormat=jsonFormat, method=method)
        return content

    def pluginsDescriptions(self,jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['plugins_descriptions'], jsonFormat=jsonFormat, method=method)
        return content

    def policyPreferencesList(self,jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['policy_preferences_list'], jsonFormat=jsonFormat, method=method)
        return content


    def policyList(self, jsonFormat=True, method="GET"):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['policy_list'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def policyDelete(self, policyId, jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['policy_delete'], jsonFormat=jsonFormat, method=method)
        return content

    def policyCopy(self, policyId, jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(params={'policy_id':policyId}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['policy_copy'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def policyEdit(self, policyData, jsonFormat=True, method="POST"):
        self.constructParamsAndHeaders(headers=policyData , jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['policy_edit'], jsonFormat=jsonFormat, method=method)
        return content
    ...
    def scanNew(self, target, policyId, scanName, jsonFormat=True, method="POST"):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['scan_new'], jsonFormat=jsonFormat, method=method)
        return content
