...
class NessusClient:
    ...
    def report2DetailsPlugin(self, reportUuid, hostname, port, protocol, severity, pluginId, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid,
                                               ...
                                               'severity':severity,
                                               'plugin_id':pluginId}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report2_details_plugin'])
        return content
    ...
    def reportTags(self, reportUuid, hostname, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid,
                                               'hostname':hostname}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_tags'])
        return content

    def reportHasAuditTrail(self, reportUuid, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_hasAuditTrail'])
        return content

    def reportAttributesList(self, reportUuid, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_attributes_list'])
        return content

    def reportErrors(self, reportUuid, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_errors'])
        return content

    def reportHasKB(self, reportUuid, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_hasKB'])
        return content

    def reportCanDeleteItems(self, reportUuid, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_canDeleteItems'])
        return content
    ...
    def report2DeleteItem(self, reportUuid, hostname, port, pluginId, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid,
                                               ...
                                               'port':port,
                                               'plugin_id':pluginId}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report2_deleteItem'])
        return content
    ...
    def reportTrailDetails(self, reportUuid, hostname, pluginId, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid,
                                               'hostname':hostname,
                                               'plugin_id':pluginId}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_trail_details'])
        return content

    def report2Vulnerabilities(self, reportUuid, jsonFormat=True):
        self.constructParamsAndHeaders(params={'report':reportUuid}, jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report2_vulnerabilities'])
        return content

    def reportChapterList(self, jsonFormat=True):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_chapter_list'])
        return content
    ...
    def reportChapter(self, reportUuid, chapters, format, token, v1=False, jsonFormat=True):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['report_chapter'])
        return content
    ...
    def reportFileImport(self, fileName, jsonFormat=True):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['report_file_import'])
        return content


    def reportFileXsltList(self, fileName, jsonFormat=True):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
        content = self.requestNessus(self.url+self.nessusFunctions['report_file_xslt_list'])
        return content
    ...
    def reportFileXslt(self, reportUuid, xslt, token, jsonFormat=True):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['report_file_xslt'])
        return content
    ...
    def reportFileXsltDownload(self, fileName, jsonFormat=True):
        ...
        content = self.requestNessus(self.url+self.nessusFunctions['report_file_xslt_download'])
        return content
