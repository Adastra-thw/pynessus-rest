...
class NessusClient:
    ...
    def serverGetCert(self,jsonFormat=False):
        self.constructParamsAndHeaders(jsonFormat=jsonFormat)
    ...
    def policyDownload(self, policyId, jsonFormat=False):
        response = requests.get(url+'?policy_id'+policyId, data=self.body, headers=self.headers, verify=self.validateCert)
        self.seqNumber += 1
        return response.content
